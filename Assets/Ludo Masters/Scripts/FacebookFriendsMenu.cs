...
public class FacebookFriendsMenu : MonoBehaviour
{
    ...
    public GameObject listLeaderboards;
    public GameObject friendPrefab;
    public GameObject friendPrefab2;
    public GameObject leaderboardPrefab2;
    ...
    private List<GameObject> LeaderboardsObjects = new List<GameObject>();

    public GameObject myLeaderboardAvatar;

    public GameObject myLeaderboardName;
    public GameObject myLeaderboardCoins;
    public GameObject myLeaderboardPosition;
    ...
    public void updateNameLeaderboards(int i, string text, string id, string coins)
    {
        List<GameObject> friendsObjects = this.LeaderboardsObjects;
        ...
        if (friendsObjects != null && friendsObjects.Count > 0 && i <= friendsObjects.Count - 1 && friendsObjects[i] != null)
        {
            friendsObjects[i].SetActive(true);
            friendsObjects[i].transform.Find("FriendName").GetComponent<Text>().text = text;
            friendsObjects[i].transform.Find("CoinsCount").GetComponent<Text>().text = coins;
            // friendsObjects[i].transform.Find("FriendAvatar").gameObject.SetActive(true);
            // getFriendImageUrl(id, friendsObjects[i].transform.Find("FriendAvatar").GetComponent<Image>(), friendsObjects[i].transform.Find("FriendAvatar"));
        }
    ...
    // public void updateCoinsLeaderboards(int i, string text, string id)
    // {
    //     List<GameObject> friendsObjects = this.LeaderboardsObjects;
    ...
    //     {
    //         friendsObjects[i].SetActive(true);
    //         friendsObjects[i].transform.Find("CoinsCount").GetComponent<Text>().text = text;
    //         // friendsObjects[i].transform.Find("FriendAvatar").gameObject.SetActive(true);
    //         // getFriendImageUrl(id, friendsObjects[i].transform.Find("FriendAvatar").GetComponent<Image>(), friendsObjects[i].transform.Find("FriendAvatar"));
    //     }
    ...
    public void updateMyLeaderboardObject(string position)
    {
        if (GameManager.Instance.avatarMy != null)
            myLeaderboardAvatar.GetComponent<Image>().sprite = GameManager.Instance.avatarMy;

        myLeaderboardName.GetComponent<Text>().text = GameManager.Instance.nameMy;
        myLeaderboardCoins.GetComponent<Text>().text = GameManager.Instance.myPlayerData.GetCoins().ToString();
        myLeaderboardPosition.GetComponent<Text>().text = position + ".";
    }



    public void addLeaderboardsToMenu(List<string> playfabIDs, List<string> playfabFBName, List<string> playfabFBID, int position)
    {
        playersAvatars = GameObject.Find("StaticGameVariablesContainer").GetComponent<StaticGameVariablesController>().avatars;

        LeaderboardsObjects = new List<GameObject>();


        leaderboardsMenu.gameObject.SetActive(true);
        //mainMenu.gameObject.SetActive(false);



        for (int i = 0; i < playfabFBName.Count; i++)
        {


            GameObject friend = Instantiate(leaderboardPrefab2, Vector3.zero, Quaternion.identity) as GameObject;
            string name = playfabFBName[i];
            if (playfabFBName[i].Length > 13)
            {
                name = playfabFBName[i].Substring(0, 12) + "...";
            ...
            if (position != -1)
            {
                friend.transform.Find("Position").GetComponent<Text>().text = "" + position + ".";
            }
            else
                friend.transform.Find("Position").GetComponent<Text>().text = "" + (i + 1) + ".";
            //friend.transform.Find("FriendName").GetComponent<Text>().text = playfabFBName[i];
            ...
            friend.transform.parent = listLeaderboards.transform;
            friend.GetComponent<RectTransform>().localScale = new Vector3(1.0f, 1.0f, 1.0f);

            LeaderboardsObjects.Add(friend);
        ...
        PlayFabClientAPI.GetUserData(getdatarequest, (result) =>
        {
            ...
            if (data[MyPlayerData.AvatarIndexKey].Value.Equals("fb"))
            {
                if (data.ContainsKey("PlayerAvatarUrl"))
                {
                    ...
                    //if (filterInputField.IsActive())
                    mainMenu.GetComponent<MonoBehaviour>().StartCoroutine(loadImage(data["PlayerAvatarUrl"].Value, image));
                }
            }
            else
            {
    ...
    public void showLeaderboards(List<string> friendsNames, List<string> friendsIDs, List<string> friendsAvatars)
    {
        leaderboardsMenu.gameObject.SetActive(true);
    }
    ...
    public void hideLeaderboards()
    {
        ...
        foreach (GameObject o in LeaderboardsObjects)
        {
            Destroy(o);
        }
