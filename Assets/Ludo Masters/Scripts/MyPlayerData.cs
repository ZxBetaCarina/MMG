...
public class MyPlayerData
{
    ...
    public void UpdateUserData(Dictionary<string, string> data)
    {
        ...
        GameManager.Instance.gameStartUserDataUpdateCounter++;

        Debug.Log("LENMGTH:" + data.Keys.Count);
        ...
        foreach (var item in data)
        {
            ...
            {
                Debug.Log("Data contains key: " + item.Key);
                this.data[item.Key].Value = item.Value;

            }

            // if (item.Key.Equals(CoinsKey))
            // {
            //     GameManager.Instance.playfabManager.UpdatePlayerLeaderboard();
            // }
        }
