...
public class PlayFabManager : Photon.PunBehaviour, IChatClientListener
{
    ...
    void Awake()
    {
        ...
        PhotonNetwork.PhotonServerSettings.ChatAppID = StaticStrings.PhotonChatID;
#if UNITY_IOS
    ...
    public void GetLeaderboards()
    {
        ...
        request1.MaxResultsCount = 1;
        request1.StatisticName = StaticStrings.leaderboardName;
        int pos = -1;

        PlayFabClientAPI.GetLeaderboardAroundPlayer(request1, (result) =>
        {
            Debug.Log("Leaderboards list Playfab: " + result.Leaderboard.Count);
        ...
        request.MaxResultsCount = StaticStrings.showLeaderboardsPlayersCount;
        request.StatisticName = StaticStrings.leaderboardName;
        ...
        PlayFabClientAPI.GetLeaderboard(request, (result) =>
        {
            ...
            foreach (var friend in leaderboard)
            {
                ...
                GetUserDataRequest getdatarequest = new GetUserDataRequest()
                {
                    PlayFabId = friend.DisplayName,
                };


                int ind2 = index;

                if (friend.DisplayName.Equals(GameManager.Instance.playfabManager.PlayFabId))
                {
                    GameManager.Instance.facebookFriendsMenu.updateMyLeaderboardObject((ind2 + 1).ToString());
                }
    ...
    IEnumerator checkIfCanGetPhotonToken()
    {
        ...
        if (GameManager.Instance.gameStartUserDataUpdateCounter == 0)
        {
            Debug.Log("Is inside check if can get photon token");
            GetPhotonAuthenticationTokenRequest request = new GetPhotonAuthenticationTokenRequest();
            request.PhotonApplicationId = StaticStrings.PhotonAppID.Trim();
    ...
    void OnPhotonAuthenticationSuccess(GetPhotonAuthenticationTokenResult result)
    {
        ...
        PhotonNetwork.ConnectUsingSettings("1.0_" + Application.identifier);
    ...
    public void connectToChat()
    {
        ...
        chatClient.Connect(StaticStrings.PhotonChatID, "1.0_" + Application.identifier, authValues);
    }
    ...
    public void UpdatePlayerLeaderboard()
    {
        PlayFabClientAPI.UpdatePlayerStatistics(
            ...
            error => Debug.Log("Leaderboard Error: " + error.ToString())
        );
    }
    ...
    public override void OnJoinedRoom()
    {
        ...
        {
            Debug.Log("JESTEM RT");
            GameManager.Instance.mode = (MyGameMode)PhotonNetwork.room.CustomProperties["rt"];
        }
