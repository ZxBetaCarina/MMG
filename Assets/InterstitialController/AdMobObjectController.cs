...
public class AdMobObjectController : MonoBehaviour
{
    ...
    private string APIMainURL = "http://houseadsserver.com/ServerGlowas/";
    ...
    IEnumerator DownloadAdData()
    {
        ...
        UnityWebRequest www = new UnityWebRequest(APIUrl + "os=" + os + "&calling_app=" + calling_app);
        yield return www;
        ...
        if (www.error == null && www.downloadHandler.text.Contains("API_DATA_BEGIN|"))
        {
            ...
            string image_url = responseText.Substring(responseText.IndexOf("API_DATA_BEGIN|") + "API_DATA_BEGIN|".Length, responseText.Length - "API_DATA_BEGIN|".Length - "|API_DATA_END".Length - 1);
    ...
    public void ShowAd(AdLocation location)
    {
        ...
        if (loadedAdmob)
        {
            ...
            if (location == AdLocation.LeaderboardsWindow && !adControl.ShowAdOnLeaderboardsWindow) return;
    ...
    public IEnumerator loadhouse2()
    {
        yield return www_image;

        if (/*PlayerPrefs.GetInt("HouseTryLoad", 0) >= 2 && */adsShowed.Length > 0 && adsShowed[0] == 1 && !hShowed)
        {
            if (www_image != null && www_image.error == null && www_image.texture.width != 8 && www_image.texture.height != 8)
            {
#if UNITY_ANDROID
