...
namespace SweetSugar.Scripts.TargetScripts.TargetSystem
{
    ...
    public class TargetCounter
    {
        ...
        public TargetCounter(GameObject _target, int _count, Object[] _extraObject, int color,
            TargetContainer argTargetLevel, bool _NotFinishUntilMoveOut,
        ...
        private int GetCountForSquares(bool subLevel)
        {
            ...
            {
                if (targetLevel.collectAction == CollectingTypes.Destroy ||
                    targetLevel.collectAction == CollectingTypes.Clear)
                {
        ...
        private int GetSquareSpreadCount(bool subLevel)
        {
            int countJelly;
            countALlsquares =
                LevelManager.THIS.fieldBoards.Sum(i => i.GetSquares().Where(x => x.IsAvailable()).Count());
            int countALl;
            if (!subLevel)
            {
                countJelly = LevelManager.THIS.fieldBoards.Sum(i => i.GetSquares().Sum(x =>
                    x.GetSpriteRenderers().Distinct().Sum(y => extraObjects.Count(z => z == y.sprite))));
                countALl = LevelManager.THIS.fieldBoards.Sum(i => i.GetSquares().Where(x => x.IsAvailable()).Count());
            }
            else
            {
                countJelly = LevelManager.THIS.field.GetSquares().Sum(x =>
                    x.GetSpriteRenderers().Sum(y => extraObjects.Count(z => z == y.sprite)));
        ...
        {
            if (!subLevel)
                return GameObject.FindObjectsOfType<TargetComponent>().Sum(x =>
                    x.GetComponent<ISquareItemCommon>().GetSpriteRenderers()
                        .Sum(y => extraObjects.Count(z => z == y.sprite)));
            return GameObject.FindObjectsOfType<TargetComponent>()
                .Where(i => i.GetComponent<IField>().GetField() == LevelManager.THIS.field).Sum(x => x
                    .GetComponent<ISquareItemCommon>()
        ...
        public void OnCheckTarget(GameObject obj, Sprite spr)
        {
            ...
            if (objects.Count() > 0 && count > 0)
            {
                ...
                if (LevelManager.THIS.animateItems.Any(i => square != null && i.linkObjectHash == square.hashCode))
                    return;
                // if(square)
                // {
        ...
        public virtual bool IsTargetReachedSublevel()
        {
            ...
            {
                if (LevelManager.THIS.field.GetItems().Count(i => i.currentType == ItemsTypes.INGREDIENT) == 0)
                    return true;
            }
        ...
        public void BindGUI(TargetGUI targetGui)
        {
            ...
            if (targetLevel.name == "Stars" && !showScores)
            {
                ...
                LeanTween.delayedCall(star, 5f,
                        () => { LeanTween.scale(star, Vector3.one * 1.2f, 0.4f).setLoopPingPong().setRepeat(4); })
                    .setRepeat(-1);
            }
        }
        ...
        public void CheckTarget(Square[] squares, bool afterDestroy = true)
        {
            ...
            {
                if (squares.SelectMany(i => i.GetSpriteRenderers().Select(x => x.sprite))
                    .Any(i => extraObjects.Any(x => x == i)))
                {
            ...
            {
                var obj = Object.FindObjectsOfType<TargetComponent>().Where(i =>
                    extraObjects.Any(x => x == i.GetComponent<SpriteRenderer>().sprite));
